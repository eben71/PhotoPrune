...
## Setup
1. Ensure `.env.example` values are configured in your environment:
   - `CLIENT_ID`
   - `CLIENT_SECRET`
   - `REDIRECT_URI`
...
## Run the scan probe
```bash
node experiments/phase1/src/scan.js --tier small --token-id default
node experiments/phase1/src/scan.js --tier medium --token-id default
node experiments/phase1/src/scan.js --tier large --token-id default
```
