...
- A production dependency graph with no known high-severity audit finding at handoff.
...
6. The local web gateway is part of the security boundary, so known high-severity Next.js/Sharp
   vulnerabilities cannot remain in the shipped production graph.
...
## 8. Production Dependency Security

The Next.js gateway is the only published application service, so its production dependency graph
is part of the PP-028 boundary rather than an unrelated maintenance concern. Builder handoff must
remove these known high-severity findings from the frozen lock graph:

| Component | Minimum patched version | Findings covered                                                                           |
| --------- | ----------------------- | ------------------------------------------------------------------------------------------ |
| Next.js   | `16.2.11`               | `GHSA-6gpp-xcg3-4w24`, `GHSA-m99w-x7hq-7vfj`, `GHSA-89xv-2m56-2m9x`, `GHSA-p9j2-gv94-2wf4` |
| Sharp     | `0.35.0`                | `GHSA-f88m-g3jw-g9cj` and the reported inherited libvips CVEs                              |

Use the newest compatible patched releases that have passed the repository's 24-hour
`minimumReleaseAge`; the floors above are not instructions to remain on those exact versions after a
newer eligible patch exists. Keep `next`, `@next/eslint-plugin-next`, and `eslint-config-next` on a
compatible aligned patch. Because Sharp is an optional transitive dependency of Next.js, retain a
root pnpm override at `>=0.35.0` when needed to keep every platform-specific lock path on the patched
line.

The lockfile is generated by pnpm and reviewed as code. Do not suppress audit findings, hand-edit
integrities, delete a production dependency only from the lockfile, or use `--no-optional` as proof
that a vulnerable supported production path is fixed. Run in this order:

1. `pnpm dependency:preflight` to confirm selected releases satisfy the publish-age policy.
2. Update manifests/overrides and regenerate `pnpm-lock.yaml` using the pinned pnpm.
3. `pnpm install --frozen-lockfile` to prove manifest/lock agreement.
4. `pnpm audit --prod --audit-level=high` to prove no high-severity production advisory remains.
5. Web lint, typecheck, tests, and production build to catch framework/native-image regressions.

If registry metadata is unavailable, the dependency repair is blocked rather than guessed: do not
commit a stale or manually fabricated lockfile. A transient registry limitation may be recorded, but
PP-028 cannot be marked Done while the high-severity production audit still fails.

## 9. Error and Observability Architecture
...
3. Upgrade Next.js/Sharp to release-age-eligible patched versions and regenerate the lockfile.
4. Prove frozen install, production high-severity audit, and web regression gates.
5. Update architecture/deployment docs required by PP-028 without absorbing PP-032's full rewrite.
6. Run focused, full, docs, and deployment-policy gates.
7. Record builder and independent verifier evidence.
...
| Dependency security       | Release-age preflight; frozen install; zero high production audit findings; web build  |
...
10. The frozen graph resolves Next.js `>=16.2.11` and Sharp `>=0.35.0`, passes release-age preflight,
    and produces zero high-severity result from `pnpm audit --prod --audit-level=high`.
...
| Published gateway retains a known high advisory     | Patched floors, release-age preflight, frozen audit gate  | Newly disclosed advisories require normal dependency response  |

## 14. Builder Review Questions
...
7. Do the selected Next.js and Sharp patches satisfy `minimumReleaseAge`, align framework lint
   packages, and produce a pnpm-generated lockfile with a clean high-severity production audit?
